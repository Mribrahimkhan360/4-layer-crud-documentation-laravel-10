{{-- resources/views/product-orders/index.blade.php --}}
...
<div class="max-w-5xl">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div class="px-7 py-5 border-b border-slate-100 flex items-center justify-between">
            <div>
                <p class="font-bold text-gray-700 text-sm">Product Orders</p>
                <p class="text-xs text-slate-500">{{ $orders->count() }} total records</p>
            </div>
            <a href="{{ route('product-orders.create') }}"
            ...
            <table class="w-full text-sm text-slate-700">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Quantity</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                {{-- Edit --}}
                                <a href="{{ route('product-orders.edit', $order->id) }}"
                                   class="text-xs font-semibold text-brand-600 hover:text-brand-700
                                          bg-brand-50 hover:bg-brand-100 px-3 py-1.5 rounded-lg
                                          transition-colors duration-150">
                                    Edit
                                </a>

                                {{-- Delete --}}
                                <form action="{{ route('product-orders.destroy', $order->id) }}"
                                      method="POST"
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-slate-400 text-sm">
                            No orders found.
                            <a href="{{ route('product-orders.create') }}"
                               class="text-brand-600 hover:underline ml-1">Add your first order.</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
