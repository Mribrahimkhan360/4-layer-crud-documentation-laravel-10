...
@section('subtitle', 'Update brand information')
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div class="px-7 py-5 border-b border-slate-100 bg-gradient-to-r from-amber-50 to-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="font-bold text-slate-800 text-sm">Edit Brand</p>
                    <p class="text-xs text-slate-500">Editing: <span class="font-semibold text-slate-700">{{ $brand->name }}</span></p>
                </div>
            </div>
        </div>

        {{-- Form --}}
        <form action="{{ route('brands.update', $brand->id) }}" method="POST" class="px-7 py-6 space-y-5">
            @csrf @method('PUT')

            <div>
                <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-2">
                    Brand Name <span class="text-red-500 normal-case tracking-normal">*</span>
                </label>
                <input
                    type="text"
                    name="name"
